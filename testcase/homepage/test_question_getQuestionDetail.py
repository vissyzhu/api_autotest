...
class Test_GetQuestionDetail(unittest.TestCase):
    ...
    def check_result(self):
        ...
        cc = connect[1]
        cc.execute(
            "SELECT detail,patient_condition,disease,ask_by FROM ask_question WHERE id=%s" % self.question_id)  # 查询答疑问题表
        result = cc.fetchall()  # 获得数据库查询结果
        conn.commit()
        conn.close()
        # 结果验证
        self.assertEqual(self.response["status"], 0, "接口连接错误")
        self.assertEqual(self.response['result']['question']['id'], self.question_id, '问题id错误')
